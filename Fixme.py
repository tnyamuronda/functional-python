...
def evens(n):
    ...
    []
    '''
    result = filter(lambda x: x % 2 == 0, range(n + 1))
    return list(result)
...
def threes(n):
    ...
    '''
    result = filter(lambda x: "3" in str(x), range(n + 1))
    return list(result)


def small_words(text):
    '''
    Returns a list of all words in the input text that are < 5 characters long.
...
def squares(n):
    ...
    '''
    result = map(lambda x: x**2, range(1, n + 1))
    return list(result)


def lengths(strings):
    '''
    Given list of strings, returns list of lengths of the corresponding strings
